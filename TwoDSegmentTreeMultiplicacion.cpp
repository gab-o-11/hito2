...
private:
    ...
    void construir(int node, int start, int end, const vector<vector<int>>& matrix) {
        if (start == end) {
            ...
            vector<int> merged(matrix[0].size(), 1);
            for (int col = 0; col < matrix[0].size(); col++) {
                merged[col] = tree[left(node)]->producto(col, col) *
                              tree[right(node)]->producto(col, col);
            }
    ...
    int consulta(int node, int start, int end, int x1, int x2, int y1, int y2) {
        if (x2 < start || end < x1) return 1;
        if (x1 <= start && end <= x2) return tree[node]->producto(y1, y2);
        int m = mid(start, end);
...
public:
    ...
    int producto(int x1, int y1, int x2, int y2) {
...
int main() {
    ...
    cout << "Producto total del rango [0,0] a [2,2]: " << tree.producto(0, 0, 2, 2) << endl;

    tree.actualizar(1, 1, 10);
    cout << "Producto despues de actualizar (1,1): " << tree.producto(0, 0, 2, 2) << endl;
